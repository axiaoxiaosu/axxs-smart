...
import com.smart.web.component.DisComponent;
...
public class MenuServiceImpl implements MenuService {
   ...
   /* @Override
    ...
    public List<MplMenu> getMenus(String username, GetMenusRequest getMenusRequest) {
        ...
        //手动排序下
        List<MplMenu> menuList = mplMenuDAO.selectMplMenu(username, menuName, types);
        if (menuList != null && !menuList.isEmpty()) {
            //按照目录和菜单手动排序下
            newList = menuList.stream().filter(e -> Integer.valueOf(e.getType()) == MplMenuEnum.MENUTYPE_1.getValue())
                    .sorted(
                            Comparator.comparing(MplMenu::getSort).reversed()
                    ).collect(Collectors.toList());

            newList.addAll(
                    menuList.stream().filter(e -> Integer.valueOf(e.getType()) == MplMenuEnum.MENUTYPE_2.getValue())
                            .sorted(
                                    Comparator.comparing(MplMenu::getSort).reversed()
                            ).collect(Collectors.toList()));

            newList.addAll(
                    menuList.stream().filter(e -> Integer.valueOf(e.getType()) == MplMenuEnum.MENUTYPE_3.getValue())
                            .sorted(
                                    Comparator.comparing(MplMenu::getSort).reversed()
                            ).collect(Collectors.toList()));
            //重新赋值 因为需要携带page参数
