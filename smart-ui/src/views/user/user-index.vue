        ...
        <el-card>
          <el-row :gutter="2">
            <el-col :xs="24" :sm="24" :md="24" :lg="2" :xl="2">
              <el-input ref="search" v-model="searchData.username" style="height: 100%;" placeholder="输入用户名称"/>
            </el-col>
...
<script>
import { getUsersPage, deleteUser, adduser } from '@/api/user.js'
...
export default {
  ...
  methods: {
    ...
    init: async function() {
      ...
      // 请求数据
      await getUsersPage().then((response) => {
    ...
    async getData() {
      const self = this
      self.tableLoading = true
      await getUsersPage(self.searchData).then((response) => {
        self.page.tableData = response.data
        self.page.size = response.pageSize
        self.page.totalPage = response.totalPage
      }).catch((e) => {
      ...
      switch (self.operateType) {
        case 1:
          self.$confirm('此操作将删除多条用户数据请谨慎操作, 是否继续?', '提示', {
            ...
            deleteUser({ userIds: userIds }).then((response) => {
